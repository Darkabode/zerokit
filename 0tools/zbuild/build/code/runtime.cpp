...
int __cdecl atexit(_PVFV pfn) {
    static UINT cAlloc;
    const UINT ALLOC_STEP = 16;

    if (cTerms>=cAlloc) {
        cAlloc += ALLOC_STEP;
        if (ppfnTerm == NULL) {
            ppfnTerm = (_PVFV*)::HeapAlloc(::GetProcessHeap()/*gHeap*/, 0, sizeof(_PVFV)*cAlloc);
        }
        else {
            ppfnTerm = (_PVFV*)::HeapReAlloc(::GetProcessHeap()/*gHeap*/, 0, ppfnTerm, sizeof(_PVFV)*cAlloc);
        }
...
void cpp_shutdown()
{
    ...
    ::HeapFree(::GetProcessHeap()/*gHeap*/, 0, ppfnTerm);
