...
namespace zgui
{
	...
	{
		if( lstrcmp(pstrName, DUI_CTR_TABLAYOUT) == 0 ) return static_cast<CTabLayoutUI*>(this);
	...
	bool CTabLayoutUI::SelectItem(int iIndex)
	{
		...
		if( _pManager != NULL ) {
			...
			_pManager->SendNotify(this, DUI_MSGTYPE_TABSELECT, m_iCurSel, iOldSel);
		}
		return true;
	}

	void CTabLayoutUI::SetAttribute(const String& pstrName, const String& pstrValue)
	{
