...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
    ...
    if(pcs && pcs->lpszName)
    {
        ...
        if(FAILED(pserv->TxSetText((TCHAR *)pcs->lpszName)))
            goto err;
#else
        size_t iLen = lstrlen(pcs->lpszName);
...
{
    if( lstrcmp(pstrName, _T("RichEdit")) == 0 ) return static_cast<CRichEditUI*>(this);
...
CDuiString CRichEditUI::GetText() const
{
    ...
    gt.codepage = 1200;
    lpText = (LPTSTR)memalloc(sizeof(TCHAR) * (lLen + 1));
...
CDuiString CRichEditUI::GetSelText() const
{
    ...
    TxSendMessage(EM_EXGETSEL, 0, (LPARAM)&cr, 0);
    LPWSTR lpText = NULL;
    lpText = (LPWSTR)memalloc(sizeof(WCHAR) *(cr.cpMax - cr.cpMin + 1));
...
CDuiString CRichEditUI::GetTextRange(long nStartChar, long nEndChar) const
{
    ...
    LPWSTR lpText = NULL;
    lpText = (LPWSTR)memalloc(sizeof(WCHAR) * (nEndChar - nStartChar + 1));
...
{
    LPWSTR lpText = NULL;
    lpText = (LPWSTR)memalloc(sizeof(WCHAR) * (nMaxLength + 1));
