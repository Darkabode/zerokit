...
STDMETHODIMP CActiveXCtrl::ShowObject(void)
{
    ...
    if( m_pViewObject != NULL ) m_pViewObject->Draw(DVASPECT_CONTENT, -1, NULL, NULL, NULL, hDC, (RECTL*) &m_pOwner->_rcItem, (RECTL*) &m_pOwner->_rcItem, NULL, NULL);
...
STDMETHODIMP CActiveXCtrl::OnInPlaceActivateEx(BOOL* pfNoRedraw, DWORD dwFlags)        
{
    ...
    if( m_pInPlaceObject != NULL ) {
        CDuiRect rcItem = m_pOwner->_rcItem;
        if( !m_bWindowless ) rcItem.ResetOffset();
...
HRESULT CActiveXCtrl::RegisterEventHandler( BOOL inAdvise )
{
	...
	hr=m_pOwner->GetControl(IID_IWebBrowser2, (void**)&pWebBrowser);
	if (FAILED(hr))
		return hr;
...
void CActiveXWnd::DoVerb(LONG iVerb)
{
    ...
    pUnk->DoVerb(iVerb, NULL, pOleClientSite, 0, _hWnd, &m_pOwner->m_pOwner->GetPos());
}
...
void CActiveXUI::SetPos(RECT rc)
{
    ...
    if( !m_pControl->m_bWindowless ) {
        ...
        ::MoveWindow(*m_pControl->m_pWindow, _rcItem.left, _rcItem.top, _rcItem.right - _rcItem.left, _rcItem.bottom - _rcItem.top, TRUE);
    }
}
...
void CActiveXUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
    ...
    {
        m_pControl->m_pViewObject->Draw(DVASPECT_CONTENT, -1, NULL, NULL, NULL, hDC, (RECTL*) &_rcItem, (RECTL*) &_rcItem, NULL, NULL); 
    }
}
...
bool CActiveXUI::DoCreateControl()
{
    ...
    if (!m_sModuleName.isEmpty()) {
        HMODULE hModule = ::LoadLibraryW(m_sModuleName.toWideCharPointer());
        if( hModule != NULL ) {
    ...
    if( _pManager != NULL ) _pManager->SendNotify((CControlUI*)this, "showactivex", 0, 0, false);
    if( (dwMiscStatus & OLEMISC_INVISIBLEATRUNTIME) == 0 ) {
        Hr = m_pUnk->DoVerb(OLEIVERB_INPLACEACTIVATE, NULL, pOleClientSite, 0, _pManager->GetPaintWindow(), &_rcItem);
...
String CActiveXUI::GetModuleName() const
{
