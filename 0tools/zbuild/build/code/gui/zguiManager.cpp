...
String CPaintManagerUI::m_pStrDefaultFontName;//added by cddjr at 05/18/2012
...
static BOOL CALLBACK enumMonitorsProc(HMONITOR, HDC, LPRECT r, LPARAM userInfo)
{
    ...
    memcpy((unsigned char*)monitorRect, (const unsigned char*)r, sizeof(RECT));
    return FALSE;
}
...
m_pParentResourcePM(NULL)
{
	...
	if (CPaintManagerUI::m_pStrDefaultFontName.length() > 0) {
		lstrcpyW(lf.lfFaceName, CPaintManagerUI::m_pStrDefaultFontName.toWideCharPointer());
	}
    ...
    EnumDisplayMonitors(0, 0, &enumMonitorsProc, (LPARAM)&_monitorRect);
}
...
CPaintManagerUI::~CPaintManagerUI()
{
    ...
    for (i = 0; i < _aAsyncNotify.GetSize(); ++i) {
        delete static_cast<TNotifyUI*>(_aAsyncNotify[i]);
    }
    ...
    if (_hdcOffscreen != NULL) {
        ::DeleteDC(_hdcOffscreen);
    }
    if (_hdcBackground != NULL) {
        ::DeleteDC(_hdcBackground);
    }
    if (_hbmpOffscreen != NULL) {
        ::DeleteObject(_hbmpOffscreen);
    }
    if (_hbmpBackground != NULL) {
        ::DeleteObject(_hbmpBackground);
    }
    if (_hdcPaint != NULL) {
        ::ReleaseDC(_hwndPaint, _hdcPaint);
    }
...
String CPaintManagerUI::GetInstancePath()
{
    if (m_hInstance == NULL) {
        return String::empty;
    }
...
String CPaintManagerUI::GetCurrentPath()
{
    TCHAR tszModule[MAX_PATH + 1] = { 0 };
...
LPRECT CPaintManagerUI::getMonitorArea()
{
...
{
    RECT rcClient = { 0 };
    ::GetClientRect(_hwndPaint, &rcClient);
...
void CPaintManagerUI::SetInitSize(int cx, int cy)
{
    ...
    if (_pRootControl == NULL && _hwndPaint != NULL) {
        ::SetWindowPos(_hwndPaint, NULL, 0, 0, cx, cy, SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
    }
}
...
void CPaintManagerUI::SetTransparent(int nOpacity)
{
    ...
    if( _hwndPaint != NULL ) {
        ...
        DWORD dwStyle = ::GetWindowLong(_hwndPaint, GWL_EXSTYLE);
        DWORD dwNewStyle = dwStyle;
        if( nOpacity >= 0 && nOpacity < 256 ) dwNewStyle |= WS_EX_LAYERED;
        else dwNewStyle &= ~WS_EX_LAYERED;
        if(dwStyle != dwNewStyle) ::SetWindowLong(_hwndPaint, GWL_EXSTYLE, dwNewStyle);
        fSetLayeredWindowAttributes(_hwndPaint, 0, nOpacity, LWA_ALPHA);
    }
}
        ...
        case WM_SYSCHAR:
            {
               ...
               CControlUI* pControl = _pRootControl->FindControl(__FindControlFromShortcut, &fs, UIFIND_ENABLED | UIFIND_ME_FIRST | UIFIND_TOP_FIRST);
               if( pControl != NULL ) {
                   pControl->SetFocus();
                   pControl->Activate();
                   return true;
...
//#endif
    ...
    TNotifyUI* pMsg = NULL;
    while (pMsg = static_cast<TNotifyUI*>(_aAsyncNotify.GetAt(0))) {
        _aAsyncNotify.Remove(0);
        ...
        case WM_CLOSE:
            {
                ...
                // focus loss...
                HWND hwndParent = GetWindowOwner(_hwndPaint);
                if (hwndParent != NULL) {
                    ::SetFocus(hwndParent);
                }
            }
        ...
        case WM_PAINT:
            {
                // Should we paint?
                RECT rcPaint = { 0 };
                if (!::GetUpdateRect(_hwndPaint, &rcPaint, FALSE)) {
                    return true;
                }
                if (_pRootControl == NULL) {
                    PAINTSTRUCT ps = { 0 };
                    ::BeginPaint(_hwndPaint, &ps);
                    ::EndPaint(_hwndPaint, &ps);
                    return true;
                }            
                ...
                if (m_bUpdateNeeded) {
                    ...
                    if (!::IsRectEmpty(&rcClient)) {
                        if (_pRootControl->IsUpdateNeeded()) {
                            ...
                            while (pControl = _pRootControl->FindControl(__FindControlFromUpdate, NULL, UIFIND_VISIBLE | UIFIND_ME_FIRST)) {
                        ...
                        if (m_bFirstLayout) {
                            ...
                            SendNotify(_pRootControl, DUI_MSGTYPE_WINDOWINIT, 0, 0, false);
                        }
                    }
                }
                ...
                if (m_bOffscreenPaint && _hbmpOffscreen == NULL) {
                    RECT rcClient = { 0 };
                    ::GetClientRect(_hwndPaint, &rcClient);
                    _hdcOffscreen = ::CreateCompatibleDC(_hdcPaint);
                    _hbmpOffscreen = ::CreateCompatibleBitmap(_hdcPaint, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top); 
                    ASSERT(_hdcOffscreen);
                    ASSERT(_hbmpOffscreen);
                }
                // Begin Windows paint
                PAINTSTRUCT ps = { 0 };
                ::BeginPaint(_hwndPaint, &ps);
                if (m_bOffscreenPaint) {
                    HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(_hdcOffscreen, _hbmpOffscreen);
                    int iSaveDC = ::SaveDC(_hdcOffscreen);
                    if (m_bAlphaBackground) {
                        if (_hbmpBackground == NULL) {
                            RECT rcClient = { 0 };
                            ::GetClientRect(_hwndPaint, &rcClient);
                            _hdcBackground = ::CreateCompatibleDC(_hdcPaint);;
                            _hbmpBackground = ::CreateCompatibleBitmap(_hdcPaint, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top); 
                            ASSERT(_hdcBackground);
                            ASSERT(_hbmpBackground);
                            ::SelectObject(_hdcBackground, _hbmpBackground);
                            ::BitBlt(_hdcBackground, ps.rcPaint.left, ps.rcPaint.top, ps.rcPaint.right - ps.rcPaint.left,
                                ps.rcPaint.bottom - ps.rcPaint.top, ps.hdc, ps.rcPaint.left, ps.rcPaint.top, SRCCOPY);
                        }
                        else
                            ::SelectObject(_hdcBackground, _hbmpBackground);
                        ::BitBlt(_hdcOffscreen, ps.rcPaint.left, ps.rcPaint.top, ps.rcPaint.right - ps.rcPaint.left,
                    ...
                    ::RestoreDC(_hdcOffscreen, iSaveDC);
                    ...
                    ::SelectObject(_hdcOffscreen, hOldBitmap);
            ...
            if( m_bUpdateNeeded ) {
                ::InvalidateRect(_hwndPaint, NULL, FALSE);
            }
            return true;
        case WM_PRINTCLIENT:
            {
                ...
                if( (lParam & PRF_CHILDREN) != 0 ) {
                    HWND hWndChild = ::GetWindow(_hwndPaint, GW_CHILD);
                    while( hWndChild != NULL ) {
                        RECT rcPos = { 0 };
                        ::GetWindowRect(hWndChild, &rcPos);
                        ::MapWindowPoints(HWND_DESKTOP, _hwndPaint, reinterpret_cast<LPPOINT>(&rcPos), 2);
        ...
        case WM_SIZE:
            {
                ...
                if( _pRootControl != NULL ) _pRootControl->NeedUpdate();
            }
            return true;
        case WM_TIMER:
            {
        ...
        case WM_MOUSEHOVER:
            {
                ...
                ::ZeroMemory(&_tooltip, sizeof(TOOLINFO));
                _tooltip.cbSize = sizeof(TOOLINFO);
                ...
                if( _hwndTooltip == NULL ) {
                    _hwndTooltip = ::CreateWindowEx(0, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, _hwndPaint, NULL, m_hInstance, NULL);
                    ::SendMessage(_hwndTooltip, TTM_ADDTOOL, 0, (LPARAM) &_tooltip);
                }
                ::SendMessage(_hwndTooltip, TTM_SETTOOLINFO, 0, (LPARAM) &_tooltip);
                ::SendMessage(_hwndTooltip, TTM_TRACKACTIVATE, TRUE, (LPARAM) &_tooltip);
            }
            return true;
        case WM_MOUSELEAVE:
            {
                if( _hwndTooltip != NULL ) ::SendMessage(_hwndTooltip, TTM_TRACKACTIVATE, FALSE, (LPARAM) &_tooltip);
                if( m_bMouseTracking ) ::SendMessage(_hwndPaint, WM_MOUSEMOVE, 0, (LPARAM) -1);
                m_bMouseTracking = false;
            }
            break;
        case WM_MOUSEMOVE:
            {
                ...
                if( !m_bMouseTracking ) {
                    ...
                    tme.dwHoverTime = _hwndTooltip == NULL ? 400UL : (DWORD) ::SendMessage(_hwndTooltip, TTM_GETDELAYTIME, TTDT_INITIAL, 0L);
        ...
        case WM_MOUSEWHEEL:
            {
                ...
                ::SendMessage(_hwndPaint, WM_MOUSEMOVE, 0, (LPARAM) MAKELPARAM(m_ptLastMousePos.x, m_ptLastMousePos.y));
            }
            break;
        case WM_CHAR:
            {
...
{
    ::InvalidateRect(_hwndPaint, &rcItem, FALSE);
}

bool CPaintManagerUI::AttachDialog(CControlUI* pControl)
{
    ASSERT(::IsWindow(_hwndPaint));
...
void CPaintManagerUI::ReapObjects(CControlUI* pControl)
{
    ...
    for (int i = 0; i < _aAsyncNotify.GetSize(); ++i) {
        TNotifyUI* pMsg = static_cast<TNotifyUI*>(_aAsyncNotify[i]);
...
bool CPaintManagerUI::AddOptionGroup(const String& pStrGroupName, CControlUI* pControl)
{
...
CStdPtrArray* CPaintManagerUI::GetOptionGroup(const String& pStrGroupName)
{
    LPVOID lp = m_mOptionGroup.Find(pStrGroupName);
    if (lp != 0) {
...
void CPaintManagerUI::RemoveOptionGroup(const String& pStrGroupName, CControlUI* pControl)
{
	LPVOID lp = m_mOptionGroup.Find(pStrGroupName);
	if (lp) {
		CStdPtrArray* aOptionGroup = static_cast<CStdPtrArray*>(lp);
...
void CPaintManagerUI::SetFocus(CControlUI* pControl)
{
    ...
    if( hFocusWnd != _hwndPaint && pControl != m_pFocus ) ::SetFocus(_hwndPaint);
    // Already has focus?
...
bool CPaintManagerUI::SetTimer(CControlUI* pControl, UINT nTimerID, UINT uElapse)
{
    ...
    for( int i = 0; i< m_aTimers.GetSize(); i++ ) {
        ...
        if( pTimer->pSender == pControl
            ...
            if( pTimer->bKilled == true ) {
                if( ::SetTimer(_hwndPaint, pTimer->uWinTimer, uElapse, NULL) ) {
    ...
    if( !::SetTimer(_hwndPaint, m_uTimerID, uElapse, NULL) ) return FALSE;
    ...
    for( int i = 0; i< m_aTimers.GetSize(); i++ ) {
            ...
            && pTimer->nLocalID == nTimerID )
        {
            if( pTimer->bKilled == false ) {
                if( ::IsWindow(_hwndPaint) ) ::KillTimer(pTimer->hWnd, pTimer->uWinTimer);
                pTimer->bKilled = true;
                return true;
            }
        }
...
void CPaintManagerUI::RemoveAllTimers()
{
    for( int i = 0; i < m_aTimers.GetSize(); i++ ) {
        ...
        if( pTimer->hWnd == _hwndPaint ) {
            if( pTimer->bKilled == false ) {
                if( ::IsWindow(_hwndPaint) ) ::KillTimer(_hwndPaint, pTimer->uWinTimer);
            }
            delete pTimer;
        }
    }
...
bool CPaintManagerUI::SetNextTabControl(bool bForward)
{
    ...
    CControlUI* pControl = _pRootControl->FindControl(__FindControlFromTab, &info1, UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
    if( pControl == NULL ) {  
        if( bForward ) {
            // Wrap around
            FINDTABINFO info2 = { 0 };
...
void CPaintManagerUI::AddDelayedCleanup(CControlUI* pControl)
{
    ...
    ::PostMessage(_hwndPaint, WM_APP + 1, 0, 0L);
}

void CPaintManagerUI::SendNotify(CControlUI* pControl, const String& pstrMessage, WPARAM wParam /*= 0*/, LPARAM lParam /*= 0*/, bool bAsync /*= false*/)
{
    TNotifyUI Msg;
...
TFontInfo* CPaintManagerUI::GetDefaultFontInfo()
{
    ...
    if (m_DefaultFontInfo.tm.tmHeight == 0) {
        HFONT hOldFont = (HFONT) ::SelectObject(_hdcPaint, m_DefaultFontInfo.hFont);
        ::GetTextMetrics(_hdcPaint, &m_DefaultFontInfo.tm);
        ::SelectObject(_hdcPaint, hOldFont);
    }
...
void CPaintManagerUI::SetDefaultFont(LPCTSTR pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic)
{
    ...
    ::ZeroMemory(&m_DefaultFontInfo.tm, sizeof(m_DefaultFontInfo.tm));
    if (_hdcPaint) {
        HFONT hOldFont = (HFONT)::SelectObject(_hdcPaint, hFont);
        ::GetTextMetrics(_hdcPaint, &m_DefaultFontInfo.tm);
        ::SelectObject(_hdcPaint, hOldFont);
    }
}
...
HFONT CPaintManagerUI::AddFont(const String& pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic)
{
    LOGFONTW lf = { 0 };
    ::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);
    lstrcpy(lf.lfFaceName, pStrFontName.toWideCharPointer());
    lf.lfCharSet = DEFAULT_CHARSET;
    lf.lfHeight = -nSize;
    if (bBold) {
        lf.lfWeight += FW_BOLD;
    ...
    if (_hdcPaint) {
        HFONT hOldFont = (HFONT) ::SelectObject(_hdcPaint, hFont);
        ::GetTextMetrics(_hdcPaint, &pFontInfo->tm);
        ::SelectObject(_hdcPaint, hOldFont);
    }
...
HFONT CPaintManagerUI::GetFont(const String& pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic)
{
...
bool CPaintManagerUI::FindFont(const String& pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic)
{
    TFontInfo* pFontInfo = NULL;
    for (int it = 0; it < m_aCustomFonts.GetSize(); ++it) {
        pFontInfo = static_cast<TFontInfo*>(m_aCustomFonts[it]);
...
int CPaintManagerUI::GetFontIndex(const String& pStrFontName, int nSize, bool bBold, bool bUnderline, bool bItalic)
{
    TFontInfo* pFontInfo = NULL;
    for (int it = 0; it < m_aCustomFonts.GetSize(); ++it) {
        pFontInfo = static_cast<TFontInfo*>(m_aCustomFonts[it]);
...
TFontInfo* CPaintManagerUI::GetFontInfo(int index)
{
    ...
    if( pFontInfo->tm.tmHeight == 0 ) {
        HFONT hOldFont = (HFONT) ::SelectObject(_hdcPaint, pFontInfo->hFont);
        ::GetTextMetrics(_hdcPaint, &pFontInfo->tm);
        ::SelectObject(_hdcPaint, hOldFont);
    }
    return pFontInfo;
...
const TImageInfo* CPaintManagerUI::GetImage(const String& bitmap)
{
...
const TImageInfo* CPaintManagerUI::GetImageEx(const String& bitmap, DWORD mask)
{
    TImageInfo* data = static_cast<TImageInfo*>(m_mImageHash.Find(bitmap));
    if (!data) {
        if (AddImage(bitmap, mask)) {
...
const TImageInfo* CPaintManagerUI::AddImage(const String& bitmap, DWORD mask)
{
    TImageInfo* data = NULL;
    
    data = CRenderEngine::LoadImage(bitmap, mask);
...
const TImageInfo* CPaintManagerUI::AddImage(const String& bitmapName, Gdiplus::Bitmap* pBitmap, int iWidth, int iHeight, bool bAlpha)
{
    if (pBitmap == NULL || iWidth <= 0 || iHeight <= 0) {
        return NULL;
    }
...
void CPaintManagerUI::ReloadAllImages()
{
    ...
    for( int i = 0; i< m_mImageHash.GetSize(); i++ ) {
        if(LPCTSTR bitmap = m_mImageHash.GetAt(i)) {
            data = static_cast<TImageInfo*>(m_mImageHash.Find(bitmap));
            if( data != NULL ) {
                pNewData = CRenderEngine::LoadImage(bitmap, data->dwMask);
                
                if( pNewData == NULL ) continue;

                if( data->pBitmap != NULL ) {
                    delete data->pBitmap;
                    //::DeleteObject(data->hBitmap);
                }
                data->pBitmap = pNewData->pBitmap;
                data->nX = pNewData->nX;
                data->nY = pNewData->nY;
    ...
    if( bRedraw && _pRootControl ) _pRootControl->Invalidate();
}
...
void CPaintManagerUI::AddDefaultAttributeList(LPCTSTR pStrControlName, LPCTSTR pStrControlAttrList)
{
	...
	if (pDefaultAttr != 0) {
        if (m_DefaultAttrHash.Find(pStrControlName) == 0) {
...
LPCTSTR CPaintManagerUI::GetDefaultAttributeList(LPCTSTR pStrControlName) const
{
    ...
    if (pDefaultAttr) {
        return pDefaultAttr->toWideCharPointer();
    }
    else {
        return NULL;
    }
...
CControlUI* CPaintManagerUI::FindControl(const String& pstrName) const
{
    ASSERT(_pRootControl);
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromName(CControlUI* pThis, LPVOID pData)
{
    ...
    return (sName.compareIgnoreCase(pstrName) == 0) ? pThis : NULL;
}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromClass(CControlUI* pThis, LPVOID pData)
{
    ...
    if( lstrcmp(pstrType, _T("*")) == 0 || lstrcmp(pstrType, pType) == 0 ) {
        int iIndex = -1;
