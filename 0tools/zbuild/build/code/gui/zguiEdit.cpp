...
namespace zgui
{
	...
	void CEditWnd::Init(CEditUI* pOwner)
	{
		...
		SetWindowFont(_hWnd, m_pOwner->GetManager()->GetFontInfo(m_pOwner->GetFont())->hFont, TRUE);
		Edit_LimitText(_hWnd, m_pOwner->GetMaxChar());
		if( m_pOwner->IsPasswordMode() ) Edit_SetPasswordChar(_hWnd, m_pOwner->GetPasswordChar());
		Edit_SetText(_hWnd, m_pOwner->GetText());
		Edit_SetModify(_hWnd, FALSE);
		SendMessage(EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN, MAKELPARAM(0, 0));
		Edit_Enable(_hWnd, m_pOwner->IsEnabled() == true);
		Edit_SetReadOnly(_hWnd, m_pOwner->IsReadOnly() == true);
		//Styls
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if( uMsg == WM_KILLFOCUS ) lRes = OnKillFocus(uMsg, wParam, lParam, bHandled);
            ...
            if (GET_WM_COMMAND_CMD(wParam, lParam) == EN_CHANGE) {
				...
				::InvalidateRect(_hWnd, &rcClient, FALSE);
			}
		}
	...
	{
		LRESULT lRes = ::DefWindowProc(_hWnd, uMsg, wParam, lParam);
	...
	LRESULT CEditWnd::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		// Copy text back
		int cchLen = ::GetWindowTextLength(_hWnd) + 1;
		LPTSTR pstr = static_cast<LPTSTR>(memalloc(cchLen * sizeof(TCHAR)));
        if (pstr == NULL) {
            return 0;
        }
		::GetWindowText(_hWnd, pstr, cchLen);
		m_pOwner->_text = pstr;
	...
	{
		if( lstrcmp(pstrName, DUI_CTR_EDIT) == 0 ) return static_cast<CEditUI*>(this);
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK || event.Type == UIEVENT_RBUTTONDOWN) 
		{
			if( IsEnabled() ) {
				...
				if( IsFocused() && m_pWindow == NULL )
				{
					...
					if( PtInRect(&_rcItem, event.ptMouse) )
					{
				...
				else if( m_pWindow != NULL )
				{
					...
					pt.x -= _rcItem.left + m_rcTextPadding.left;
					pt.y -= _rcItem.top + m_rcTextPadding.top;
	...
	{
		_text = pstrText;
		if( m_pWindow != NULL ) Edit_SetText(*m_pWindow, _text);
		Invalidate();
	}
	...
	void CEditUI::SetVisible(bool bVisible)
	{
		...
		if( !IsVisible() && m_pWindow != NULL ) _pManager->SetFocus(NULL);
	}
	...
	{
		if( _cxyFixed.cy == 0 ) return CSize(_cxyFixed.cx, _pManager->GetFontInfo(GetFont())->tm.tmHeight + 6);
		return CControlUI::EstimateSize(szAvailable);
	}

	void CEditUI::SetAttribute(const String& pstrName, const String& pstrValue)
	{
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		CDuiString sText = _text;
		if( m_bPasswordMode ) {
			sText.Empty();
			LPCTSTR p = _text.GetData();
			while( *p != _T('\0') ) {
		...
		if( IsEnabled() ) {
			CRenderEngine::DrawText(hDC, _pManager, rc, sText, m_dwTextColor, \
				m_iFont, DT_SINGLELINE | m_uTextStyle);
		}
		else {
			CRenderEngine::DrawText(hDC, _pManager, rc, sText, m_dwDisabledTextColor, \
				m_iFont, DT_SINGLELINE | m_uTextStyle);
