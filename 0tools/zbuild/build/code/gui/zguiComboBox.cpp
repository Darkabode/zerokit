...
namespace zgui
{
	...
	void CComboBoxUI::SetAttribute(const String& pstrName, const String& pstrValue)
	{
        if (pstrName == "arrowimage") {
	...
	void CComboBoxUI::PaintStatusImage(HDC hDC)
	{
		...
		else
		{
			...
			CDuiRect rcDest(0, 0, _rcItem.right - _rcItem.left, _rcItem.bottom - _rcItem.top);
