...
static BOOL WINAPI AlphaBitBlt(HDC hDC, int nDestX, int nDestY, int dwWidth, int dwHeight, HDC hSrcDC, int nSrcX, int nSrcY, int wSrc, int hSrc, BLENDFUNCTION ftn)
{
    ...
    lpbiSrc = (LPBITMAPINFO)memalloc(sizeof(BITMAPINFOHEADER));
	if (lpbiSrc == NULL) {
		::DeleteDC(hTempDC);
		return FALSE;
	}
    ...
    // Fill in the BITMAPINFOHEADER
    lpbiDest = (LPBITMAPINFO)memalloc(sizeof(BITMAPINFOHEADER));
	if (lpbiDest == NULL) {
        memfree(lpbiSrc);
...
TImageInfo* CRenderEngine::LoadImage(const String& bitmap, DWORD mask)
{
    LPBYTE pData = NULL;
    DWORD dwSize = 0;
    bool bAlphaChannel = false;

    Gdiplus::Bitmap* pBitmap = Resources::getInstance()->getImage(bitmap.toUTF8());

    if (pBitmap == 0) {
        return 0;
    }

    bAlphaChannel = (pBitmap->GetFlags() & Gdiplus::ImageFlagsHasAlpha) != 0;

    TImageInfo* data = new TImageInfo;
    data->pBitmap = pBitmap;
    data->nX = pBitmap->GetWidth();
    data->nY = pBitmap->GetHeight();
...
{
	if (bitmap->pBitmap) {
        delete bitmap->pBitmap;
	}
	delete bitmap;
}

void CRenderEngine::DrawImage(HDC hDC, Gdiplus::Bitmap* pBitmap, const RECT& rc, const RECT& rcPaint, const RECT& rcBmpPart, const RECT& rcCorners, bool alphaChannel, BYTE uFade, bool hole, bool xtiled, bool ytiled)
{
    if (pBitmap == 0) {
        return;
    }
        ...
        if (!hole) {
            ...
            if (::IntersectRect(&rcTemp, &rcPaint, &rcDest)) {
                if (!xtiled && !ytiled) {
                    ...
                    Gdiplus::Rect r(rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
                    g.DrawImage(pBitmap, r, rcBmpPart.left + rcCorners.left, rcBmpPart.top + rcCorners.top,
                        rcBmpPart.right - rcBmpPart.left - rcCorners.left - rcCorners.right,
                        rcBmpPart.bottom - rcBmpPart.top - rcCorners.top - rcCorners.bottom, Gdiplus::UnitPixel);
...
//                     rcBmpPart.bottom - rcBmpPart.top - rcCorners.top - rcCorners.bottom, bf);
                    ...
                    for( int j = 0; j < iTimesY; ++j ) {
                        ...
                        for( int i = 0; i < iTimesX; ++i ) {
                            ...
                            Gdiplus::Rect r(rcDest.left + lWidth * i, rcDest.top + lHeight * j, lDestRight - lDestLeft, lDestBottom - lDestTop);
                            g.DrawImage(pBitmap, r, rcBmpPart.left + rcCorners.left, rcBmpPart.top + rcCorners.top, lDrawWidth, lDrawHeight, Gdiplus::UnitPixel);
...
//                                 rcBmpPart.left + rcCorners.left, rcBmpPart.top + rcCorners.top, lDrawWidth, lDrawHeight, bf);
                    ...
                    for( int i = 0; i < iTimes; ++i ) {
                        ...
                        Gdiplus::Rect r(lDestLeft, rcDest.top, lDestRight - lDestLeft, rcDest.bottom);
                        g.DrawImage(pBitmap, r, rcBmpPart.left + rcCorners.left, rcBmpPart.top + rcCorners.top, \
                            lDrawWidth, rcBmpPart.bottom - rcBmpPart.top - rcCorners.top - rcCorners.bottom, Gdiplus::UnitPixel);
...
//                             lDrawWidth, rcBmpPart.bottom - rcBmpPart.top - rcCorners.top - rcCorners.bottom, bf);
                    ...
                    for( int i = 0; i < iTimes; ++i ) {
                        ...
                        Gdiplus::Rect r(rcDest.left, rcDest.top + lHeight * i, rcDest.right, lDestBottom - lDestTop);
                        g.DrawImage(pBitmap, r, rcBmpPart.left + rcCorners.left, rcBmpPart.top + rcCorners.top, \
                            rcBmpPart.right - rcBmpPart.left - rcCorners.left - rcCorners.right, lDrawHeight, Gdiplus::UnitPixel);
...
//                             rcBmpPart.right - rcBmpPart.left - rcCorners.left - rcCorners.right, lDrawHeight, bf);                    
        ...
        if( rcCorners.left > 0 && rcCorners.top > 0 ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                ...
                Gdiplus::Rect r(rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
                g.DrawImage(pBitmap, r, rcBmpPart.left, rcBmpPart.top, rcCorners.left, rcCorners.top, Gdiplus::UnitPixel);
//                 lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC,
//                     rcBmpPart.left, rcBmpPart.top, rcCorners.left, rcCorners.top, bf);
        ...
        if( rcCorners.top > 0 ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                ...
                Gdiplus::Rect r(rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
                g.DrawImage(pBitmap, r, rcBmpPart.left + rcCorners.left, rcBmpPart.top, rcBmpPart.right - rcBmpPart.left - \
                    rcCorners.left - rcCorners.right, rcCorners.top, Gdiplus::UnitPixel);
//                 lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC, \
...
//                     rcCorners.left - rcCorners.right, rcCorners.top, bf);
        ...
        if( rcCorners.right > 0 && rcCorners.top > 0 ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                ...
                Gdiplus::Rect r(rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
                g.DrawImage(pBitmap, r, rcBmpPart.right - rcCorners.right, rcBmpPart.top, rcCorners.right, rcCorners.top, Gdiplus::UnitPixel);
//                 lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC, \
//                     rcBmpPart.right - rcCorners.right, rcBmpPart.top, rcCorners.right, rcCorners.top, bf);
        ...
        if( rcCorners.left > 0 ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                ...
                Gdiplus::Rect r(rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
                g.DrawImage(pBitmap, r, rcBmpPart.left, rcBmpPart.top + rcCorners.top, rcCorners.left, rcBmpPart.bottom - \
                    rcBmpPart.top - rcCorners.top - rcCorners.bottom, Gdiplus::UnitPixel);
//                 lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC, \
...
//                     rcBmpPart.top - rcCorners.top - rcCorners.bottom, bf);
        ...
        if( rcCorners.right > 0 ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                ...
                Gdiplus::Rect r(rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
                g.DrawImage(pBitmap, r, rcBmpPart.right - rcCorners.right, rcBmpPart.top + rcCorners.top, rcCorners.right, \
                    rcBmpPart.bottom - rcBmpPart.top - rcCorners.top - rcCorners.bottom, Gdiplus::UnitPixel);
//                 lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC, \
...
//                     rcBmpPart.bottom - rcBmpPart.top - rcCorners.top - rcCorners.bottom, bf);
        ...
        if( rcCorners.left > 0 && rcCorners.bottom > 0 ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                ...
                Gdiplus::Rect r(rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
                g.DrawImage(pBitmap, r, rcBmpPart.left, rcBmpPart.bottom - rcCorners.bottom, rcCorners.left, rcCorners.bottom, Gdiplus::UnitPixel);
//                 lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC, \
//                     rcBmpPart.left, rcBmpPart.bottom - rcCorners.bottom, rcCorners.left, rcCorners.bottom, bf);
        ...
        if( rcCorners.bottom > 0 ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                ...
                Gdiplus::Rect r(rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
                g.DrawImage(pBitmap, r, rcBmpPart.left + rcCorners.left, rcBmpPart.bottom - rcCorners.bottom, \
                    rcBmpPart.right - rcBmpPart.left - rcCorners.left - rcCorners.right, rcCorners.bottom, Gdiplus::UnitPixel);
//                 lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC, \
...
//                     rcBmpPart.right - rcBmpPart.left - rcCorners.left - rcCorners.right, rcCorners.bottom, bf);
        ...
        if( rcCorners.right > 0 && rcCorners.bottom > 0 ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                ...
                Gdiplus::Rect r(rcDest.left, rcDest.top, rcDest.right, rcDest.bottom);
                g.DrawImage(pBitmap, r, rcBmpPart.right - rcCorners.right, rcBmpPart.bottom - rcCorners.bottom, rcCorners.right, \
                    rcCorners.bottom, Gdiplus::UnitPixel);
//                 lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC, \
    ...
    else 
    {
            ...
            && rcCorners.left == 0 && rcCorners.right == 0 && rcCorners.top == 0 && rcCorners.bottom == 0)
        {
            if( ::IntersectRect(&rcTemp, &rcPaint, &rc) ) {
                Gdiplus::Rect r(rcTemp.left, rcTemp.top, rcTemp.right - rcTemp.left, rcTemp.bottom - rcTemp.top);
                g.DrawImage(pBitmap, r, rcBmpPart.left + rcTemp.left - rc.left, rcBmpPart.top + rcTemp.top - rc.top, rcTemp.right - rcTemp.left, rcTemp.bottom - rcTemp.top, Gdiplus::UnitPixel);
...
//                             rcBmpPart.bottom - rcBmpPart.top - rcCorners.top - rcCorners.bottom, SRCCOPY);
                        ...
                        for( int j = 0; j < iTimesY; ++j ) {
                            ...
                            for( int i = 0; i < iTimesX; ++i ) {
                                ...
                                Gdiplus::Rect r(rcDest.left + lWidth * i, rcDest.top + lHeight * j, lDestRight - lDestLeft, lDestBottom - lDestTop);
                                g.DrawImage(pBitmap, r, rcBmpPart.left + rcCorners.left, rcBmpPart.top + rcCorners.top, lDestRight - lDestLeft, lDestBottom - lDestTop, Gdiplus::UnitPixel);
...
bool DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const String& sImageName, RECT rcItem, RECT rcBmpPart, RECT rcCorner, DWORD dwMask, BYTE bFade, bool bHole, bool bTiledX, bool bTiledY)
{
	if (sImageName.isEmpty()) {
		return false;
	}
	const TImageInfo* data = NULL;
	data = pManager->GetImageEx(sImageName, dwMask);
    if (!data) {
        return false;
    }
	...
	CRenderEngine::DrawImage(hDC, data->pBitmap, rcItem, rcPaint, rcBmpPart, rcCorner, data->alphaChannel, bFade, bHole, bTiledX, bTiledY);

	return true;
}

bool CRenderEngine::DrawImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const String& imageName, const String& modifyImageName)
{
    ...
    // 1、aaa.jpg
    // 2、file='aaa.jpg' res='' dest='0,0,0,0' source='0,0,0,0' corner='0,0,0,0' 
    ...
    for (int i = 0; i < 2; ++i, image_count = 0) {
        ...
        for (int j = 0; j < attrs.size(); ++j) {
            ...
            if (keyVal[0] == "file") {
                if (image_count > 0) {
                    zgui::DrawImage(hDC, pManager, rc, rcPaint, currImageName, rcItem, rcBmpPart, rcCorner, dwMask, bFade, bHole, bTiledX, bTiledY);
                }
            ...
            else if (keyVal[0] == "mask") {
                dwMask = keyVal[1].getHexValue32();
            }
                ...
                bTiledX = ((keyVal[1] == "true"));
            }
            else if (keyVal[0] == "ytiled") {
                bTiledY = ((keyVal[1] == "true"));
            }
        }
    }
...
void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, const String& pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
    ...
    ::DrawTextW(hDC, pstrText.toWideCharPointer(), -1, &rc, uStyle | DT_NOPREFIX);
    ::SelectObject(hDC, hOldFont);
}

void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, const String& text, DWORD dwTextColor, RECT* prcLinks, String* sLinks, int& nLinkRects, UINT uStyle)
{
    ...
    LPCTSTR pstrText = text.toWideCharPointer();
    ...
    while (*pstrText != _T('\0')) {
        ...
        else if (!bInRaw && ( *pstrText == _T('<') || *pstrText == _T('{')) && (pstrText[1] >= _T('a') && pstrText[1] <= _T('z'))
                ...
                {    
                    ...
                    if (sName.isEmpty()) { // Italic
						...
						if (sImageString.contains("file=\'")) {
							String sImageName;
							LPCTSTR pStrImage = sImageString.toWideCharPointer();
							String sItem;
							String sValue;
						...
						if (pImageInfo) {
                            ...
                            if( pt.x + iWidth > rc.right && pt.x > rc.left && (uStyle & DT_SINGLELINE) == 0 ) {
                                ...
                                if( bDraw && bLineDraw ) {
                                    ...
                                    DrawImage(hDC, pImageInfo->pBitmap, rcImage, rcImage, rcBmpPart, rcCorner, \
