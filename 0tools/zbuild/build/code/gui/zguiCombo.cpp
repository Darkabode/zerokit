...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        if (pControl && lstrcmp(pControl->GetClass(), _T("ScrollBarUI")) != 0) {
        ...
        if( _hWnd != (HWND) wParam ) PostMessage(WM_CLOSE);
    }
...
{
    if (lstrcmp(pstrName, DUI_CTR_COMBO) == 0) {
        return static_cast<CComboUI*>(this);
    }
    if (lstrcmp(pstrName, _T("IListOwner")) == 0) {
...
bool CComboUI::SelectItem(int iIndex, bool bTakeFocus)
{
    ...
    if( _pManager != NULL ) _pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, m_iCurSel, iOldSel);
    Invalidate();
...
{
    if( _cxyFixed.cy == 0 ) return CSize(_cxyFixed.cx, _pManager->GetDefaultFontInfo()->tm.tmHeight + 12);
...
bool CComboUI::Activate()
{
    ...
    ASSERT(m_pWindow);
    m_pWindow->Init(this);
    if( _pManager != NULL ) _pManager->SendNotify(this, DUI_MSGTYPE_DROPDOWN);
    Invalidate();
    return true;
}
...
void CComboUI::SetAttribute(const String& pstrName, const String& pstrValue)
{
