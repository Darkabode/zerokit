...
int FCGX_PutChar(int c, FCGX_Stream *stream)
{
    if(stream->wrNext != stream->stop)
        return (*stream->wrNext++ = (uint8_t) c);
    if(stream->isClosed || stream->isReader)
        return EOF;
        ...
        int paddingLength)
{
    ...
    header.requestIdB1      = (uint8_t) ((requestId     >> 8) & 0xff);
    header.requestIdB0      = (uint8_t) ((requestId         ) & 0xff);
    header.contentLengthB1  = (uint8_t) ((contentLength >> 8) & 0xff);
    header.contentLengthB0  = (uint8_t) ((contentLength     ) & 0xff);
    header.paddingLength    = (uint8_t) paddingLength;
    header.reserved         =  0;
    return header;
}
        ...
        int protocolStatus)
{
    ...
    body.appStatusB3    = (uint8_t) ((appStatus >> 24) & 0xff);
    body.appStatusB2    = (uint8_t) ((appStatus >> 16) & 0xff);
    body.appStatusB1    = (uint8_t) ((appStatus >>  8) & 0xff);
    body.appStatusB0    = (uint8_t) ((appStatus      ) & 0xff);
    body.protocolStatus = (uint8_t) protocolStatus;
 ...
 *----------------------------------------------------------------------
 */
static uint8_t *AlignPtr8(uint8_t *p) {
...
typedef struct FCGX_Stream_Data {
    uint8_t *buff;      /* buffer after alignment */
    int bufflen;              /* number of bytes buff can store */
    uint8_t *mBuff;     /* buffer as returned by Malloc */
    uint8_t *buffStop;  /* reader: last valid byte + 1 of entire buffer.
        ...
        FCGX_Request *reqDataPtr, int bufflen, int isReader, int streamType)
{
    ...
    data->mBuff = (uint8_t *)malloc(bufflen);
...
void FCGX_FreeStream(FCGX_Stream **streamPtr)
{
    ...
    if(stream == NULL) {
	printf("FCGX_FreeStream: AAAAAAAAAAAAAAAAAAAAAAAAAaaaaaaaaaaaaaaaaaaaaaa");
        return;
    }
