...
#define ZFS_KEYBBLOCK                           ((40 << ZFS_FUNCTION_SHIFT) | ZFS_MODULE_FILE)
...
typedef unsigned int zfs_handle_t;
...
*/
EXTERN_C int __stdcall zuserio_init(unsigned char key[CLIENT_KEY_SIZE], unsigned char clientId[CLIENT_ID_SIZE]);

/** Освобождает все ресурсы и завершает работу с ZFS. */
EXTERN_C void __stdcall zuserio_shutdown();
...
/** Устанавливает/снимает хук с клавиатуры.

    Возвращает код ошибки (0 - успешно).
*/
EXTERN_C int __stdcall userio_keyboard_hook(int setHook);

/** Блокирует клавиатуру.

    Возвращает код ошибки (0 - успешно).
*/
EXTERN_C int __stdcall userio_keyboard_block(int isBlock);
