...
__inline unsigned long __cdecl skip_to(const char *format) 
{
    unsigned long nr;
...
static int __cdecl write_pad(unsigned int* dlen,struct arg_printf* fn, unsigned int len, int padwith) {
    int nr=0;
...
int __cdecl __ltostr(char *s, unsigned int size, unsigned long i, unsigned int base, int UpCase)
{
    char *tmp;
    unsigned int j=0;
...
int __cdecl __v_printf(struct arg_printf* fn, const char *format, va_list arg_ptr)
{
  unsigned int len=0;
...
static int __cdecl swrite(void*ptr, size_t nmemb, struct str_data* sd) {
...
int __cdecl vsnprintf(char* str, size_t size, const char *format, va_list arg_ptr) 
{
    int n;
...
int __cdecl vsprintf(char *dest,const char *format, va_list arg_ptr)
{
...
int __cdecl sprintf(char *buffer, const char *format, ...)
{
    ...
    ///*HRESULT */StringCchVPrintf(buffer, STRSAFE_MAX_CCH, format, argptr);
...
int __cdecl _snprintf(char *dest, size_t n, const char *fmt, ...)
{
    int retValue;
	va_list args;
	...
	va_start(args, fmt);
	retValue = vsprintf(dest, fmt, args);
    //StringCchVPrintf(dest, STRSAFE_MAX_CCH, fmt, args);
	va_end(args);
	return retValue;
}
...
// {
// 	n;
//     //StringCchVPrintf(dest, n, fmt, args);
