...
#include "zguiArrayAllocationBase.h"
#include "zguiElementComparator.h"
...
public:
    ...
    */
    ...
    {
        return isPositiveAndBelow(index, numUsed) ? data.elements [index] : ElementType();
    }
    ...
    */
    void insert(int indexToInsertAt, ParameterType newElement)
    {
        ...
        if (isPositiveAndBelow (indexToInsertAt, numUsed)) {
            ...
            if (numberToMove > 0) {
                fn_RtlMoveMemory(insertPos + 1, insertPos, ((size_t)numberToMove) * sizeof(ElementType));
            }
    ...
    void insertMultiple(int indexToInsertAt, ParameterType newElement, int numberOfTimesToInsertIt)
    {
        if (numberOfTimesToInsertIt > 0) {
            ...
            if (isPositiveAndBelow (indexToInsertAt, numUsed)) {
                ...
                const int numberToMove = numUsed - indexToInsertAt;
                fn_RtlMoveMemory(insertPos + numberOfTimesToInsertIt, insertPos, ((size_t) numberToMove) * sizeof (ElementType));
            }
            else {
    ...
    void insertArray (int indexToInsertAt, const ElementType* newElements, int numberOfElements)
    {
        if (numberOfElements > 0) {
            ...
            if (isPositiveAndBelow(indexToInsertAt, numUsed)) {
                ...
                const int numberToMove = numUsed - indexToInsertAt;
                fn_RtlMoveMemory(insertPos + numberOfElements, insertPos, numberToMove * sizeof (ElementType));
            }
            else {
                insertPos += numUsed;
            }
	...
	*/
	...
	{
		int num = 0;
		for (const Type* const* e = elementsToAdd; *e != 0; ++e) {
			++num;
		}
    ...
    */
	template <class OtherArrayType>
	void swapWith(OtherArrayType& otherArray) throw()
    {
    ...
    */
    ElementType remove(const int indexToRemove)
    {
        if (isPositiveAndBelow (indexToRemove, numUsed)) {
            ...
            if (numberToShift > 0) {
                fn_RtlMoveMemory(e, e + 1, ((size_t) numberToShift) * sizeof (ElementType));
            }
    ...
    */
    ...
    {
        const int endIndex = zgui_limit(0, numUsed, startIndex + numberToRemove);
        startIndex = zgui_limit(0, numUsed, startIndex);
        ...
        if (endIndex > startIndex) {
            ...
            if (numToShift > 0) {
                fn_RtlMoveMemory(e, e + numberToRemove, ((size_t) numToShift) * sizeof (ElementType));
            }
    ...
    */
    void swap (const int index1, const int index2)
    {
        if (isPositiveAndBelow (index1, numUsed) && isPositiveAndBelow (index2, numUsed)) {
            swapVariables (data.elements [index1], data.elements [index2]);
        }
    }
    ...
    void move (const int currentIndex, int newIndex) throw()
    {
        if (currentIndex != newIndex) {
            if (isPositiveAndBelow (currentIndex, numUsed)) {
                ...
                __movsb((uint8_t*)tempCopy, (const uint8_t*)(data.elements + currentIndex), sizeof (ElementType));

                if (newIndex > currentIndex) {
                    fn_RtlMoveMemory(data.elements + currentIndex, data.elements + currentIndex + 1, sizeof (ElementType) * (size_t) (newIndex - currentIndex));
                }
                else {
                    fn_RtlMoveMemory(data.elements + newIndex + 1, data.elements + newIndex, sizeof (ElementType) * (size_t) (currentIndex - newIndex));
                }

                __movsb((uint8_t*)(data.elements + newIndex), (const uint8_t*)tempCopy, sizeof (ElementType));
            }
        }
    }
...
private:
    ...
    inline void deleteAllElements() throw()
    {
        for (int i = 0; i < numUsed; ++i) {
    ...
    void minimiseStorageAfterRemoval()
    {
        if (data.numAllocated > numUsed * 2) {
            data.shrinkToNoMoreThan (max (numUsed, 64 / (int) sizeof (ElementType)));
        }
    }
};
