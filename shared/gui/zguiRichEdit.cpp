...
typedef HRESULT (__stdcall *FnCreateTextServices)(IUnknown *punkOuter, ITextHost *pITextHost, IUnknown **ppUnk);
FnCreateTextServices _fnCreateTextServices = 0;
...
m_re(NULL)
{
    ...
    HMODULE hLib = LoadLibraryA("msftedit.dll");

    if (hLib != NULL) {
        _fnCreateTextServices = (FnCreateTextServices)GetProcAddress(hLib, "CreateTextServices");
    }
}
...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
    ...
    if(_fnCreateTextServices == 0 || FAILED(_fnCreateTextServices(NULL, this, &pUnk))) {
        goto err;
    }
...
BOOL CTxtWinHost::TxShowScrollBar(INT fnBar, BOOL fShow)
{
    ...
    if( fnBar == SB_VERT && pVerticalScrollBar ) {
        ...
        pHorizontalScrollBar->aetVisible(fShow == TRUE);
    }
    else if( fnBar == SB_BOTH ) {
        if( pVerticalScrollBar ) pVerticalScrollBar->aetVisible(fShow == TRUE);
...
const String& CRichEditUI::getClass() const
{
    return "RichEditUI";
}

LPVOID CRichEditUI::getInterface(const String& name)
{
    if (name == "RichEdit") {
...
CDuiString CRichEditUI::GetText() const
{
    ...
    gt.codepage = 1200;
    lpText = (LPTSTR)fn_memalloc(sizeof(TCHAR) * (lLen + 1));
...
CDuiString CRichEditUI::GetSelText() const
{
    ...
    TxSendMessage(EM_EXGETSEL, 0, (LPARAM)&cr, 0);
    LPWSTR lpText = NULL;
    lpText = (LPWSTR)fn_memalloc(sizeof(WCHAR) *(cr.cpMax - cr.cpMin + 1));
...
CDuiString CRichEditUI::GetTextRange(long nStartChar, long nEndChar) const
{
    ...
    LPWSTR lpText = NULL;
    lpText = (LPWSTR)fn_memalloc(sizeof(WCHAR) * (nEndChar - nStartChar + 1));
...
{
    LPWSTR lpText = NULL;
    lpText = (LPWSTR)fn_memalloc(sizeof(WCHAR) * (nMaxLength + 1));
...
void CRichEditUI::DoInit()
{
    ...
    if( m_pTwh ) {
        ...
        _pManager->AddMessageFilter(this);
    }
    ...
    if( m_pTwh ) {
        if( msg == WM_KEYDOWN && TCHAR(wparam) == VK_RETURN ) {
            if( !m_bWantReturn || (::GetKeyState(VK_CONTROL) < 0 && !m_bWantCtrlReturn) ) {
                if( _pManager != NULL ) _pManager->SendNotify((CControlUI*)this, "return");
                return S_OK;
            }
        }
...
void CRichEditUI::SetScrollPos(SIZE szPos)
{
    ...
    if( cy != 0 ) {
        int iPos = 0;
        if( m_pTwh && !m_bRich && _pVerticalScrollBar && _pVerticalScrollBar->IsVisible() ) 
            iPos = _pVerticalScrollBar->GetScrollPos();
        WPARAM wParam = MAKEWPARAM(SB_THUMBPOSITION, _pVerticalScrollBar->GetScrollPos());
        TxSendMessage(WM_VSCROLL, wParam, 0L, 0);
        if( m_pTwh && !m_bRich && _pVerticalScrollBar && _pVerticalScrollBar->IsVisible() ) {
            if( cy > 0 && _pVerticalScrollBar->GetScrollPos() <= iPos )
                _pVerticalScrollBar->SetScrollPos(iPos);
        }
    }
    if( cx != 0 ) {
        WPARAM wParam = MAKEWPARAM(SB_THUMBPOSITION, _pHorizontalScrollBar->GetScrollPos());
...
void CRichEditUI::LineDown()
{
    ...
    if( m_pTwh && !m_bRich && _pVerticalScrollBar && _pVerticalScrollBar->IsVisible() ) {
        if( _pVerticalScrollBar->GetScrollPos() <= iPos )
            _pVerticalScrollBar->SetScrollPos(_pVerticalScrollBar->GetScrollRange());
    }
}
...
void CRichEditUI::SetPos(RECT rc)
{
    ...
    if( m_pTwh ) {
        ...
        if( bVScrollBarVisiable && (!_pVerticalScrollBar->IsVisible() || m_bVScrollBarFixing) ) {
            LONG lWidth = rc.right - rc.left + _pVerticalScrollBar->GetFixedWidth();
            LONG lHeight = 0;
            SIZEL szExtent = { -1, -1 };
...
{
    RECT rcTemp = { 0 };
    if( !::IntersectRect(&rcTemp, &rcPaint, &_rcItem) ) return;
    ...
    if( m_items.GetSize() > 0 ) {
        ...
        if( !::IntersectRect(&rcTemp, &rcPaint, &rc) ) {
            for( int it = 0; it < m_items.GetSize(); it++ ) {
                ...
                if( pControl ->IsFloat() ) {
                    if( !::IntersectRect(&rcTemp, &_rcItem, &pControl->GetPos()) ) continue;
...
{
    if (lstrcmp(pstrName, _T("vscrollbar")) == 0) {
        if (lstrcmp(pstrValue, _T("true")) == 0) {
            m_lTwhStyle |= ES_DISABLENOSCROLL | WS_VSCROLL;
        }
    }
    if (lstrcmp(pstrName, _T("autovscroll")) == 0) {
        if (lstrcmp(pstrValue, _T("true")) == 0) {
            m_lTwhStyle |= ES_AUTOVSCROLL;
        }
    }
    else if (lstrcmp(pstrName, _T("hscrollbar")) == 0) {
        if (lstrcmp(pstrValue, _T("true")) == 0) {
            m_lTwhStyle |= ES_DISABLENOSCROLL | WS_HSCROLL;
        }
    }
    if (lstrcmp(pstrName, _T("autohscroll")) == 0) {
        if (lstrcmp(pstrValue, _T("true")) == 0) {
            m_lTwhStyle |= ES_AUTOHSCROLL;
        }
    }
    else if (lstrcmp(pstrName, _T("wanttab")) == 0) {
        SetWantTab(lstrcmp(pstrValue, _T("true")) == 0);
    }
    else if( lstrcmp(pstrName, _T("wantreturn")) == 0 ) {
        SetWantReturn(lstrcmp(pstrValue, _T("true")) == 0);
    }
    else if( lstrcmp(pstrName, _T("wantctrlreturn")) == 0 ) {
        SetWantCtrlReturn(lstrcmp(pstrValue, _T("true")) == 0);
    }
    else if( lstrcmp(pstrName, _T("rich")) == 0 ) {
        SetRich(lstrcmp(pstrValue, _T("true")) == 0);
    }
    else if( lstrcmp(pstrName, _T("multiline")) == 0 ) {
        if( lstrcmp(pstrValue, _T("false")) == 0 ) m_lTwhStyle &= ~ES_MULTILINE;
    }
    else if( lstrcmp(pstrName, _T("readonly")) == 0 ) {
        if( lstrcmp(pstrValue, _T("true")) == 0 ) { m_lTwhStyle |= ES_READONLY; m_bReadOnly = true; }
    }
    else if( lstrcmp(pstrName, _T("password")) == 0 ) {
        if( lstrcmp(pstrValue, _T("true")) == 0 ) m_lTwhStyle |= ES_PASSWORD;
    }
    else if( lstrcmp(pstrName, _T("align")) == 0 ) {
    ...
    else if( lstrcmp(pstrName, _T("font")) == 0 ) SetFont(_ttoi(pstrValue));
    else if( lstrcmp(pstrName, _T("textcolor")) == 0 ) {
