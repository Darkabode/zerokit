...
#define DT_KEEP   3
	...
	void CDateTimeWnd::Init(CDateTimeUI* pOwner)
	{
		...
		::SendMessage(_hWnd, DTM_SETSYSTEMTIME, 0, (LPARAM)&m_pOwner->m_sysTime);
	...
	LRESULT CDateTimeWnd::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		{
			::SendMessage(_hWnd, DTM_GETSYSTEMTIME, 0, (LPARAM)&m_pOwner->m_sysTime);
	...
	const String& CDateTimeUI::getClass() const
	{
		return "DateTimeUI";
	}

	LPVOID CDateTimeUI::getInterface(const String& name)
	{
        if (name == "DateTime") {
            return static_cast<CDateTimeUI*>(this);
        }
		return CLabelUI::getInterface(name);
	}
