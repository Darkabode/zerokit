...
static int LzInWindow_Create(CMatchFinder *p, uint32_t keepSizeReserv)
{
    ...
    if (p->bufferBase == 0 || p->blockSize != blockSize)
    {
        LzInWindow_Free(p);
        p->blockSize = blockSize;
        p->bufferBase = (uint8_t*)fn_memalloc((size_t)blockSize);
    }
...
static CLzRef* AllocRefs(uint32_t num)
{
    ...
    if (sizeInBytes / sizeof(CLzRef) != num)
        return 0;
    return (CLzRef*)fn_memalloc(sizeInBytes);
}
...
void LzmaEnc_SaveState(CLzmaEncHandle pp)
{
  ...
  int i;
  __movsb((uint8_t*)&dest->lenEnc, (const uint8_t*)&p->lenEnc, sizeof(p->lenEnc));
  __movsb((uint8_t*)&dest->repLenEnc, (const uint8_t*)&p->repLenEnc, sizeof(p->repLenEnc));
  dest->state = p->state;

  for (i = 0; i < kNumStates; i++)
  {
    __movsb((uint8_t*)dest->isMatch[i], (const uint8_t*)p->isMatch[i], sizeof(p->isMatch[i]));
    __movsb((uint8_t*)dest->isRep0Long[i], (const uint8_t*)p->isRep0Long[i], sizeof(p->isRep0Long[i]));
  }
  for (i = 0; i < kNumLenToPosStates; ++i) {
    __movsb((uint8_t*)dest->posSlotEncoder[i], (const uint8_t*)p->posSlotEncoder[i], sizeof(p->posSlotEncoder[i]));
  }
  __movsb((uint8_t*)dest->isRep, (const uint8_t*)p->isRep, sizeof(p->isRep));
  __movsb((uint8_t*)dest->isRepG0, (const uint8_t*)p->isRepG0, sizeof(p->isRepG0));
  __movsb((uint8_t*)dest->isRepG1, (const uint8_t*)p->isRepG1, sizeof(p->isRepG1));
  __movsb((uint8_t*)dest->isRepG2, (const uint8_t*)p->isRepG2, sizeof(p->isRepG2));
  __movsb((uint8_t*)dest->posEncoders, (const uint8_t*)p->posEncoders, sizeof(p->posEncoders));
  __movsb((uint8_t*)dest->posAlignEncoder, (const uint8_t*)p->posAlignEncoder, sizeof(p->posAlignEncoder));
  __movsb((uint8_t*)dest->reps, (const uint8_t*)p->reps, sizeof(p->reps));
  __movsb((uint8_t*)dest->litProbs, (const uint8_t*)p->litProbs, (0x300 << p->lclp) * sizeof(uint32_t));
}
...
void LzmaEnc_RestoreState(CLzmaEncHandle pp)
{
  ...
  __movsb((uint8_t*)dest->posAlignEncoder, (const uint8_t*)p->posAlignEncoder, sizeof(p->posAlignEncoder));
  __movsb((uint8_t*)dest->reps, (const uint8_t*)p->reps, sizeof(p->reps));
  __movsb((uint8_t*)dest->litProbs, (const uint8_t*)p->litProbs, (0x300 << dest->lclp) * sizeof(uint32_t));
}
...
static int RangeEnc_Alloc(CRangeEnc *p)
{
  if (p->bufBase == 0)
  {
    p->bufBase = (uint8_t*)fn_memalloc(RC_BUF_SIZE);
    if (p->bufBase == 0)
      return 0;
...
static int LzmaEnc_Alloc(CLzmaEnc *p, uint32_t keepWindowSize)
{
...

  {
    ...
    {
      LzmaEnc_FreeLits(p);
      p->litProbs = (uint32_t *)fn_memalloc((0x300 << lclp) * sizeof(uint32_t));
      p->saveState.litProbs = (uint32_t *)fn_memalloc((0x300 << lclp) * sizeof(uint32_t));
...
int lzma_decode(uint8_t* outBuffer, uint32_t* pOutSize, const uint8_t* inBuffer, uint32_t inSize, ELzmaStatus* status)
{
    ...
    *pOutSize = 0;
    ...
    probs = (uint32_t*)fn_memalloc(((uint32_t)LZMA_BASE_SIZE + (LZMA_LIT_SIZE << (lc + lp))) << 2);
    if (probs == NULL) {
        return SZ_ERROR_MEM;
    }
